...
def handle_dante_menu():
    while True:
        ...
        if choice == '0':
            ...
            src_utils.clear_screen()
            print("═" * 40)
            print("🔄 SYNC SOCKSIFY WITH PSIPHON".center(40))
            print("═" * 40)
            ...
            if missing:
                ...
                if not result or "LocalSocksProxyPort" not in result:
                    src_utils.error("Failed to detect Psiphon SOCKS port!",
                                    solution="Ensure Psiphon is running and its config is available")
                else:
                    param = result["LocalSocksProxyPort"]
                    socks_ip = "127.0.0.1"
                    socks_port = param.get("value")
                    valid = param.get("valid", False)

                    if not valid or not socks_port:
                        src_utils.error("Invalid Psiphon SOCKS configuration!",
                                        solution="Check if Psiphon is running and port is valid")
                    else:
                        if update_dante_config(socks_ip, socks_port):
                            src_utils.success(f"Successfully synced with Psiphon SOCKS: {socks_ip}:{socks_port}")
                        else:
                            src_utils.error("Failed to update Socksify config", 
                                            solution="Check Socksify's socks.conf syntax")
